...
# Demo route that shows what the vulnerability was (without actually allowing directory browsing)
@app.route('/browse/')
@app.route('/browse/<path:path>')
def browse_directories(path=''):
    """
    This route previously demonstrated directory listing vulnerability.
    Now it returns a secure error response.
    """
    return render_template_string('''
    <html>
    <head>
        ...
        <style>
            ...
            .alert { background-color: #dff0d8; border-left: 4px solid #5cb85c; padding: 10px; margin: 15px 0; }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>Access Denied</h1>
            <div class="alert">
                <strong>Security Fix Applied:</strong> Directory browsing has been disabled.
                Previously this demo allowed directory listing, which exposed sensitive files.
            </div>
            <p>Directory listing is now blocked - this is the secure configuration.</p>
...
def decompile_demo():
    """
    This route previously allowed viewing decompiled source code.
    Now it shows a secure message explaining the fix.
    """
    return render_template_string('''
    <html>
    <head>
        <title>Security Fix Applied</title>
        <style>
            body { font-family: Arial, sans-serif; margin:0; padding: 20px; line-height: 1.6; }
            .container { max-width: 800px; margin:0 auto; background-color: #f9f9f9; padding: 20px; border-radius: 5px; }
            h1 { color: #5cb85c; }
    ...
    <body>
        <div class="container">
            <h1>Security Fix Applied</h1>
            
            <div class="alert">
                <strong>Fixed:</strong> Sensitive files are no longer accessible. 
                Previously this demo allowed viewing decompiled source code with hardcoded credentials.
            </div>
            
            <h2>What was fixed:</h2>
            <ul>
                <li>Directory browsing has been disabled</li>
                <li>Sensitive files are no longer served</li>
                <li>Source code with credentials is not exposed</li>
            </ul>
